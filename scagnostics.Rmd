...
## Data description
The model describes a neutron star merger and contains 6 free parameters, i.e. each neutron star is described by its mass and radius, and a so-called tidal deformability parameter $\Lambda$. A posterior sample containing 2538 points is available @ligoData and can be used to constrain the parameters. For details about the modelling and statistical procedure see @Abbott2018exr.
Show corner plot here, comment on well constrained relation between m1 and m2 ("chirp mass")
For further study we therefore remove m2 from the dataset and use the guided tour working with the remaining 5 dimensions.

Use spline2d index, find relation between 3 parameters, ask Rory for interpretation
