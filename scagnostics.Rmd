...
```{r tourpath}
data(mtcars)
t1 <- save_history(mtcars, grand_tour(2), max=4)
t1full <- as.list(interpolate(t1))
n <- length(t1full)
sc <- NULL
for (i in 1:n) {
  dprj <- as.matrix(mtcars) %*% t1full[[i]]
...
```{r plotscag, fig.width=6, fig.height=7, fig.cap="Short tour paths with scagnostics computed on projections. Most of the indices exhibit sharp jumps in scagnostic value."}
