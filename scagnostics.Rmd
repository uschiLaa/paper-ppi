...
In general we require that a good ppi should be smooth over geodesically interpolated projections, and be fast to calculate for dynamic viewing of the optimization with the guided tour. Note that there is a trade-off between smoothness requirements and the efficiency of the optization, and index functions sensitive to random small fluctuations may be viable in conjunction with an approriate optimization strategy, as noted above. Moreover, larger calculation times may be acceptable when we do not require observing the optimization in real-time, but instead record the guided tour which can be replayed once the optimization has converged. 
