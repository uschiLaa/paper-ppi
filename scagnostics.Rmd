...
\textcolor{red}{\cite{posse95b} lists five criteria: minimized by normal distribution, approximately affine invariant, consistent, resistent to features in the tail, fast computation. Comment that different index values for the same plane will make optimisation less efficient. \cite{jones87} moreover lists rotation and translation invariance (independence of choice of basis in the plane), and also mentions conditions for numeric optimisation, i.e. continuous and depending on optimisation methods also differentiable. Interesting: according to \cite{jones87} the original index proposed by Friedman and Tukey did not have property of rotation invariance.}
  ...
  - smoothness: the index function should evolve smoothly between nearby projections, i.e. on an interpolated path we expect to observe slow and consistent change in the index function. On the other hand if two similar projections produce very different index values, i.e. we observe sharp jumps when looking at the evolution of the index function, and in particular if those fluctuations dominate the overall evolution, the index is not suitable for projection pursuit. We evaluate smoothness both for transitions between noise variables and when moving from noise variables to an informative view in Section \ref{sec:smooth}.
