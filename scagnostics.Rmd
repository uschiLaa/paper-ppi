...
- **spiral**:  $p$ points where $n-2$ points are independently drawn from a normal distribution, and dirction $n-1$ and $n$ have points selected on an Archimedean spiral, i.e. $r = a + b \theta$, with $a=b=0.1$ and we sample angles $\theta$ from a normal distribution with mean 0 and variance $2\pi$ and taking the absolute for well defined spiral shape with higher density at smaller radius.

Two samples sizes are also used: $p=100$ (small) and $p=1000$ (large).
...
\begin{table}
\begin{center}
\caption{Summary of findings, showing to what extend the considered index functions pass the criteria for a good PPI. Checkmark symbols good behaviour, exclamation mark symbols some issues while cross symbols failure on the corresponding criteria. Each index has particular strengths and drawbacks and selection must be guided by the considered example, see text for details.}
\label{tab:summary}
...
Convex & ! & ! & ! & ! & \checkmark \\
Skinny & ! & !& ! & ! & \checkmark \\
Stringy & $\times$ & $\times$ & ! & ! & \checkmark \\
\hline
splines2D & \checkmark & \checkmark & ! & $\times$ & \checkmark \\
dcor2D & \checkmark & \checkmark & ! & $\times$ & ! \\
\hline
MINE & ! & \checkmark  & \checkmark & ! & ! \\
\hline
\end{tabular}
\end{center}
\end{table}
