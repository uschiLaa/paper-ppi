...
- A guided tour, showing the optimisation path, starting from $x_1 - x_2$, for one index, shows how effectively the maximum is attained. The values for other PPIs is computed for this path also, which allows an assessment of similarity between PPIs. 
- Rotation invariance is checked by computing PPIs on rotations of the structured projection. 
- Benchmarking is used to assess the computational speed.
...
Table \ref{indexTable} compares the PPIs for structured projections against those for nuisance variables. XXX adjust the text below once re-working of table is done.
...
getIdx <- function(dataT, size, structured){
  ...
  sortedList <- lapply(list(skinny, stringy, convex, micidx, ticidx, dcor2D, splines2D), sort)
  res <- tableTemp() %>%
    add_row(index="Skinny", data=dataT, size=size,
            structured=ifelse(structured, "yes", "no"),
            value5=sortedList[[1]][5], value95=sortedList[[1]][95]) %>%
    add_row(index="Stringy", data=dataT, size=size,
            structured=ifelse(structured, "yes", "no"),
            value5=sortedList[[2]][5], value95=sortedList[[2]][95]) %>%
    add_row(index="Convex", data=dataT, size=size,
            structured=ifelse(structured, "yes", "no"),
            value5=sortedList[[3]][5], value95=sortedList[[3]][95]) %>%
    add_row(index="MICe", data=dataT, size=size,
            structured=ifelse(structured, "yes", "no"),
            value5=sortedList[[4]][5], value95=sortedList[[4]][95]) %>%
    add_row(index="TICe", data=dataT, size=size,
            structured=ifelse(structured, "yes", "no"),
            value5=sortedList[[5]][5], value95=sortedList[[5]][95]) %>%
    add_row(index="dcor2D", data=dataT, size=size,
            structured=ifelse(structured, "yes", "no"),
            value5=sortedList[[6]][5], value95=sortedList[[6]][95]) %>%
    add_row(index="splines2D", data=dataT, size=size,
            structured=ifelse(structured, "yes", "no"),
            value5=sortedList[[7]][5], value95=sortedList[[7]][95])
}
...
if(!file.exists("cache/indexTable.rda")){
  ...
  for(ds in c("Pipe", "Sine", "Spiral")){
    for (size in c(100, 1000)){
      for (structured in c(TRUE, FALSE)){
        res <- full_join(res,getIdx(ds, size, structured))
      }
    }
  }
...
res <- res %>%
  mutate(Range=paste0("[",formatC(value5, digits=2, format ="f"),",",
                      formatC(value95, digits=2, format = "f"),"]")) %>%
  select(-value5, -value95) %>%
  spread(key=structured, value = Range) %>%
  rename(nuisance=no, structured=yes) %>%
  arrange(data) %>%
  select(-data)
  

kable(res,  "latex", booktabs = T, longtable = T, caption = "Summary of index values.", digits=2) %>%
  kable_styling(latex_options = c("repeat_header")) %>%
  group_rows("Pipe", 1, 14) %>%
  group_rows("Sine", 15, 28) %>%
  group_rows("Spiral", 29, 42)
```
