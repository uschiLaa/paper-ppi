...
## Rotation dependence
\textcolor{red}{testin rotation dependence by turning sine dataset with $\alpha \in [0,2\pi]$}
Interesting to note that some indexes are symmetric around $\pi$, but scagnostics indexes are very arbitrary, dependence is introduced via binnibng (and outlier removal?) here. Observe huge drops for some of the indexes! 
```{r rotationDep, fig.width=6, fig.height=7, fig.cap="Rotation dependence of the considered index functions tested on the sine 1000 dataset, by rotating the special view by an angle alpha."}
...
for (a in seq(0,2*pi, pi/20)){
  ...
  sc <- add_row(sc, holes=holesRes, cmass=cmassRes,
                ...
                stringy=scagRes[,"Stringy"], dcor2d=dcorRes,
                splines2d=splineRes, MIC=mineRes$MIC,
                TIC=mineRes$TIC, alpha=a)
  }

scMelt <- melt(sc, id=c("alpha"))
ggplot(scMelt, aes(x=alpha, y=value)) +
  geom_line() + 
...
p1 <- ggplot(dProj, aes(PP1, PP2)) + geom_point(alpha = 0.05) + theme(aspect.ratio=1) + 
  geom_path(data=getCircle(0,0.6,0.32,0.22), aes(x=x, y=y)) + 
...
p1 <- ggplot(as.tibble(dProj), aes(PP1, PP2))+geom_point(alpha=0.1)+ theme(aspect.ratio=1) + 
  geom_path(data=getCircle(0.5,0.6,0.3,0.18), aes(x=x, y=y)) + 
...
Figure \ref{fig:bbhSimulation} shows a scatterplot matrix analogue to that shown in Figure \ref{fig:neutronStarSPLOM} but where we have removed a subset of less informative parameters to obtain a clear picture. Strong association can be seen between the parameters ra, dec and time, where the first two describe the position of the event in the sky, and time is the merging time (in GPS units). Moreover, the relation between dec and time is not functional, which motivates us to use the TIC index for this example (a drawback is however the run time, this data sample contains 10k points, making optimisation of the MINE index functions very inefficient). In general we observe non-standard distributions for all variables, as well as various patterns of association between sets of variables.
\textcolor{red}{These patterns are representative of the likelihood function, since most priors are flat, or build such to capture growth with volume rather than distance. This is still efficient thanks to special sampling techniques that allow jumps in parameter space.}

### Data pre-pocessing
We follow the same steps of data pre-processing as described above for the first example. First, since constraints on the masses of two systems observed via gravitational waves are strongly correlated, we again remove the smaller mass m2 from the set of parameters. We then rescale all parameters to the range from 0 to 1.

### Applying PP

As a first test we use the full dataset and compare results obtained with different index functions. Here we expect to recover the relation between the correlated variables dec, time and ra. We compare the final view using three different metrics in Figure \ref{fig:bbhGuided}, showing the views with maximum of the TIC index (left), spines2D index (middle) and for comparison the local maximum found when optimising 1-convex (right). Indeed the TIC and splines2D index identify dec, time and ra parameters and show views of the 2-d band that is embedded in three dimensions \textcolor{red}{maybe having tour of those three dimensions would be helpful here}. Notably, the exact views differ based on the index function selected, optimising TIC is showing us the band "from the side". On the other hand, the index based on the scagnostics member convex is too noisy to find any views of interest, and the final  view is essentially equal to the starting plane in this case. All index values for the three final views are listed in Table \ref{tab:indexTableBBH}, where we see that the splines2D index does not change between the first two views, while the TIC index is increased for the first view compared the second. Finally the value of 1-convex is clearly larger for the first view as compared to the final view, but the optimisation did not succeed for this noisy index function.

We next follow a similar strategy as above and remove the variables with the strongest association from the dataset to understand if additional structure is encoded in the distribution. Removing thus time, dec and ra variable from the input, we optimise again the TIC, splines2d and 1-convex index functions. The results are shown in Fig. \ref{fig:bbhGuided2}. In this case the TIC index (left) identifies a view presenting seemingly linear correlation, originating however from one well constrained direction orthogonal to the main distribution of points. This direction is defined by the chi\_tot and chi\_p variables, for which we can read off the strong correlation in Fig. \ref{fig:bbhSimulation}. This direction is also found by the splines2D index (middle), which in addition identified the bimodal structure in the data stemming from the degeneracy between two plausible values for theta\_jn. (Interestingly the hole index in two dimensions does not identify this view, but is expected to find it in a 1-d guided tour.) Finally, as shown on the right, 1-convex does not identify any views of interest here.

We next investigate the dependence of the final result on the input starting plane. In particular if there are many local maxima the effect may be large, and many random starting planes may be needed to find a global maximum. To this end we first reduce the dataset dimensionality to include only two types of parameters: first, spin related parameters (i.e. alpha, theta\_jn, chi\_tot and chi\_p), and second position related parameters (i.e. ra, dec and distance). Note that this includes a number of variables for which we have established association and one dimensional structure above, i.e. ra and dec; chi\_tot and chi\_p; and theta\_jn. We generate a small set of random starting planes and optimise the splines2D index (it is prefered here over the TIC index as it is much more efficient to compute on large data sets). We compare six final views identified in this manner (i.e. differing only in randomly selected starting plane) in Fig. \ref{fig:bbhGuided3}. We note large variation in final views, only one of the six tries identified the view of ra vs dec, which however clearly has the maximum index value amongst all views. \textcolor{red}{We see that all final fiews pick up some combination of parameters for which we expect structure, but the structure of the data distribution and corresponding index function dependence is such that one easily gets stuck in local maxima. One way out could be the scouting phase discussed above, however this requires some additional improvements, because non-rotation-invariance of the distribution and index function make this approach intractable here.}

Finally, given the results above, it makes sense to also consider smaller parameter-subspaces to look for new association. One subspace for which we found strong association between three variables is using the randomly selected parameter set (m1, ra, chi\_tot, alpha, distance, dec). Optimising again the splines2D index value allows us to find the final view shown in Figure \ref{constructedExample} (left) \textcolor{red}{(The small constribution of alpha is hidden in the axes here, middle plot is without rescaling, therefore same size contribution from dec an alpha to y axis)}, which we can reproduce from the original parameters as shown in the middle, as a combination of the position parameters ra and dec with the spin parameter alpha. For comparison we also show the dependence between ra and dec only, which is fuzzy compared to the ideal view identified by the PP optimisation. \textcolor{red}{This result is unexpected, it can be understood from ????? (Does it mean that knowing the precession angle gives more precise information about the location? Knowing ra uniquely determines dec-alpha rather than dec it seems.)}
...
```{r bbhGuided, results="hide", fig.cap="Final views identified by the guided tour using the TIC index (left), the splines2d index (middle) or 1-convex (right).", dev = "png", dpi=300}
...
p1 <- ggplot(dProj1, aes(PP1,PP2)) + geom_point(alpha = 0.02) + theme(aspect.ratio=1) + 
  geom_path(data=getCircle(0.4,0.5,0.26,0.24), aes(x=x, y=y)) + 
  xlim(0.1, 1.4) + ylim(-0.3, 0.9) +
  geom_segment(data=getAxes(fProj1, 0.4,0.5,0.26,0.24), aes(x=x1, xend=x2, y=y1, yend=y2)) +
  geom_text(data=getAxes(fProj1, 0.4,0.5,0.26,0.24), aes(x=x2, y=y2, label=label), size=2.5)
...
p2 <- ggplot(dProj2, aes(PP1,PP2)) + geom_point(alpha = 0.02) + theme(aspect.ratio=1) + 
  geom_path(data=getCircle(0.3,-1,0.24,0.22), aes(x=x, y=y)) + 
  xlim(-0.1, 1.1) + ylim(-1.3, -0.2) +
  geom_segment(data=getAxes(fProj2,0.3,-1,0.24,0.22), aes(x=x1, xend=x2, y=y1, yend=y2)) +
  geom_text(data=getAxes(fProj2, 0.3,-1,0.24,0.22), aes(x=x2, y=y2, label=label), size=2.5)
...
p3 <- ggplot(dProj3, aes(PP1,PP2)) + geom_point(alpha = 0.02) + theme(aspect.ratio=1) + 
  geom_path(data=getCircle(0.75, 1, 0.3, 0.28), aes(x=x, y=y)) + 
  xlim(-0.2, 1.3) + ylim(-0.1, 1.3) +
  geom_segment(data=getAxes(fProj3,0.75, 1, 0.3, 0.28), aes(x=x1, xend=x2, y=y1, yend=y2)) +
  geom_text(data=getAxes(fProj3, 0.75, 1, 0.3, 0.28), aes(x=x2, y=y2, label=label), size=2.5)
grid.arrange(p1, p2, p3, ncol=3) 
```
...
```{r bbhGuided2, results="hide", fig.cap="Final views identified by the guided tour using the TIC index (left), the splines2d index (middle) or 1-convex (right).", dev = "png", dpi=300}
...
p1 <- ggplot(dProj, aes(PP1,PP2)) + geom_point(alpha = 0.02) + theme(aspect.ratio=1) + 
  geom_path(data=getCircle(1, 0, 0.34, 0.25), aes(x=x, y=y)) + 
...
p2 <- ggplot(dProj, aes(PP1,PP2)) + geom_point(alpha = 0.02) + theme(aspect.ratio=1)+ 
  geom_path(data=getCircle(0.75, 0, 0.25, 0.24), aes(x=x, y=y)) + 
...
p3 <- ggplot(dProj, aes(PP1,PP2)) + geom_point(alpha = 0.02) + theme(aspect.ratio=1)+ 
  geom_path(data=getCircle(0, 1.25, 0.32, 0.32), aes(x=x, y=y)) + 
...
```{r bbhGuided3, results="hide", fig.cap=paste("Final views identified in the dataset considering the seven dimensional parameter space (alpha, theta\\_jn, chi\\_tot, chi\\_p, ra, dec, distance), differing only by randomly selected starting plane. The correponding index values for the six views are: (", toString(format(idxL, digits=2)), ")."), dev = "png", dpi=300}
...
for (r in resList){
  ...
  idxL <- c(idxL, splines2d(dProj$PP1, dProj$PP2))
  pList[[i]] <- ggplot(dProj, aes(PP1,PP2)) + geom_point(alpha = 0.02) + theme(aspect.ratio=1)+ 
  geom_path(data=getCircle(xcents[i],ycents[i],xscales[i],yscales[i]), aes(x=x, y=y)) + 
...
grid.arrange(pList[[1]], pList[[2]], pList[[3]],pList[[4]], pList[[5]], pList[[6]], ncol=3)
```
...
p1 <- ggplot(as.tibble(dProj), aes(PP1, PP2))+geom_point(alpha=0.02) + theme(aspect.ratio=1)+ 
  geom_path(data=getCircle(-0.7, 0.6, 0.2, 0.24), aes(x=x, y=y)) + 
