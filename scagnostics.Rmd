...
\textcolor{red}{\cite{posse95a} uses simulations with "strongly hidden structure", i.e. truncated Gaussian distributions with approximately identity covariance matrix, such that they don't appear in any of the principal component planes.}
...
In the following we work with rescaled data where each direction has been centered and scaled to have variance equal to 1. This is neccessary for standard indices to be well defined, but does not impact the results when using the newly considered index functions. \textcolor{red}{not yet implemented for traces of guided tour paths I think...}
