...
## Smoothness
We first study the smoothness of the scagnostics measures calculated on a sequence of 2-d projections obtained via an interpolated grand tour path. The temporal trace of a selected set of measures is shown below, where each time step refers to an interpolation step.
...
```{r getProj}
getProj <- function(df, tPath, nameStr){
  sc <- NULL
  n <- length(tPath)
  df <- rescale(df)
  for (i in 1:n) {
  dprj <- df %*% tPath[[i]]
  #FIXME not only scagnostics but all relevant!
  #also keep track of t=i, and nameStr
  #after want to be able to directly plot full comparison.
