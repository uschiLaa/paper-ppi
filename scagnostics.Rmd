...
resp <- res %>% filter(size == 1000, data == "Pipe") %>%
  select(index, type, value95, value5) %>%
  rename(lower=value95, upper=value5)
...
res_all <- bind_cols(resp, resi, ress) %>%
  ...
  mutate(index = fct_relevel(index, "convex", "skinny", "stringy", "dcor2D", "splines2D", "MICe", "TICe")) %>%
  mutate(index = c("convex", "", "skinny", "", "stringy", "", "dcor2D", "", "splines2D", "", "MICe", "", "TICe", ""))
