...
p1 <- ggplot(dProj, aes(V1,V2)) + geom_point(alpha = 0.05) + theme(aspect.ratio=1) + 
  ...
  xlim(-1.1, 0.5) + ylim(-0.2, 1.4) +
  geom_segment(data=fProj, aes(x=cntrx, xend=PP1*sclx+cntrx, y=cntry, yend=PP2*scly+cntry))
...
p1 <- ggplot(as.tibble(dProj), aes(V1, V2))+geom_point(alpha=0.1)+ theme(aspect.ratio=1)
...
Finally, given the results above, it makes sense to also consider smaller parameter-subspaces to look for stronger association. (A procedure to first scout the parameter space for large index values is not efficent in this case because of the strong dependence on rotation angles in this paricular setup.) One subspace for which we found strong association between three variables is using the randomly selected parameter set (m1, ra, chi\_tot, alpha, distance, dec). Optimising again the splines2D index value allows us to find the final view shown in Figure \ref{constructedExample} (left), which we can reproduce from the original parameters as shown in the middle (here we work with the rescaled parameter values), as a combination of the position parameters ra and dec with the spin parameter alpha. For comparison we also show the dependence between ra and dec only, which is fuzzy compared to the ideal view identified by the PP optimisation. \textcolor{red}{This result is unexpected, it can be understood from ?????}
...
p1 <- ggplot(dProj1, aes(V1,V2)) + geom_point(alpha = 0.02) + theme(aspect.ratio=1)
iLast <- length(bbhTourFull2)
fProj2 <- bbhTourFull2[[iLast]]
dProj2 <- as.tibble(bbhM %*% fProj2)
p2 <- ggplot(dProj2, aes(V1,V2)) + geom_point(alpha = 0.02) + theme(aspect.ratio=1)
iLast <- length(bbhTourFull3)
fProj3 <- bbhTourFull3[[iLast]]
dProj3 <- as.tibble(bbhM %*% fProj3)
p3 <- ggplot(dProj3, aes(V1,V2)) + geom_point(alpha = 0.02) + theme(aspect.ratio=1)
grid.arrange(p1, p2, p3, ncol=3) 
```

```{r indexTableBBH}
getIdxComp <- function(dProj, n){
  ticI <- mine(dProj$V1, dProj$V2)$TIC
  splineI <- splines2d(dProj$V1, dProj$V2)
...
```{r bbhGuided2, results="hide", fig.cap="Final views identified by the guided tour using the TIC index (left), the splines2d index (middle) or 1-convex (right)."}
...
for (r in resList){
  ...
  idxL <- c(idxL, splines2d(dProj$V1, dProj$V2))
  pList[[i]] <- ggplot(dProj, aes(V1,V2)) + geom_point(alpha = 0.02) + theme(aspect.ratio=1)
  i <- i+1
}
grid.arrange(pList[[1]], pList[[2]], pList[[3]],pList[[4]], pList[[5]], pList[[6]], ncol=3)
```

```{r constructedExample, results="hide", fig.cap="Selecting subset of 6 parameters, for which spline index can be used to improve constraint on parameter combination wrt ra vs dec."}
...
p1 <- ggplot(as.tibble(dProj), aes(V1, V2))+geom_point(alpha=0.02) + theme(aspect.ratio=1)
p2 <- ggplot(as.tibble(bbhC),aes(dec-alpha/5, ra)) + geom_point(alpha = 0.02) + theme(aspect.ratio=1)
p3 <- ggplot(as.tibble(bbhC),aes(dec, ra)) + geom_point(alpha = 0.02) + theme(aspect.ratio=1)
grid.arrange(p1, p2, p3, ncol=3)
```
