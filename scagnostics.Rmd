...
There are a number of properties that a PPI should have, which are:


\textcolor{red}{\cite{posse95b} lists five criteria: minimized by normal distribution, approximately affine invariant, consistent, resistent to features in the tail, fast computation. Comment that different index values for the same plane will make optimisation less efficient. \cite{jones87} moreover lists rotation and translation invariance (independence of choice of basis in the plane), and also mentions conditions for numeric optimisation, i.e. continuous and depending on optimisation methods also differentiable. Interesting: according to \cite{jones87} the original index proposed by Friedman and Tukey did not have property of rotation invariance.}
  ...
  - speed: the computation time is an important factor in particular when the guided tour is used to view the optimisation in real time. Note that longer computation times may be acceptable when first recording the optimisation and replaying the guided tour once the maximum has been identified. Computation times for some benchmark scenarios are compared in Section \ref{sec:timer}.
    
##Simulation study

Three data simulations are used for examining the behaviour of the index functions. Each generates special structure in two variables, and the remaining variables are noise. This construction means that variable selection would be sufficient for the identification of the structure. We use this setup for simplicity, and since all variable combinations are treated equally in the PP it does not affect our conclusions about the ability of identifying the structure in arbitrary linear combinations of variables.
...
- index values for pair of variables, explicitly comparing the uninformative and special view of each dataset, as well as discussing the dependence on the sample size, see Section \ref{sec:dataOv}
- tour between pairs of variables, considering a planned tour that moves from the $V_1 - V_2$ view either to the $V_3 - V_4$ view (no special structure encountered over the full path) or to the $V_5 - V_6$ (moving from uninformed to special view), see Section \ref{sec:smooth}
- optimisation path, starting from $V_1 - V_2$, how can we use the available index functions and optimisation algorithms to identify the special view, see Section \ref{sec:guided}
- rotating a dataset with functional dependence to evaluate the rotation dependence of the index functions, see Section \ref{sec:rot}
- benchmarking the computing time in Section \ref{sec:timer}
...
- sphere:  $p$ points on an n-dimensional sphere randomly selected using the geozoo package [@geozoo], as an example of dataset without special views, labelled "sphere"
- pipe: $p$ points where $n-1$ points are independently drawn from a uniform distribution between $[-1,1]$, and enforcing $x_n^2 + x_{n-1}^2 = 1 \pm 0.1$ by rejection sampling as an example of a dataset with special view that is not described by functional dependence, and where sampling boundaries are apparent in the distribution, labelled "pipe"
- sine:  $p$ points where $n-1$ points are independently drawn from a normal distribution with mean 0 and variance 1, and with $x_n = \sin(x_{n-1}) + \mathrm{jittering}$ as an example dataset with special view described by a function, labelled "sine"
...
## Index traces over sequence of interpolated projections
\label{sec:smooth}
...
### Moving into informative projections
We repeat the same exercise, but now we are interested in how the index values change when going from the view in V1 vs V2 to the view in V5 vs V6, which will also allow us to view the evolution between typical low and high values of the index function. Results are shown in Figure \ref{fig:plannedtour}.
...
## Rotation dependence
\label{sec:rot}
\textcolor{red}{testing rotation dependence by turning sine dataset with $\alpha \in [0,2\pi]$}
...
ggplot(dfTimer, aes(x = n, y = t, color=name)) + 
  geom_point(alpha=0.05) +
  geom_point(data=timerMeans) +
  geom_line(data=timerMeans) +
  scale_x_log10() +
  scale_y_log10() +
...
Our results can be summarised by comparing 

- scagnostics indices:
    - advantages: scaling with larger datasets thanks to binning, set of index functions covering different types of interesting structures
    - disadvantage: noisy, requires good optimisation algorithm, stringy too noisy to be used as index
- splines2D:
    - advantages: fast to compute, also considering scaling with larger data sets, efficient in detecting functional relations, typically smooth behaviour (but we have seen some exceptions)
    - disadvantage: not general, highly not rotation invariant
- dcor2D:
    - advantages: smoothly changing, sensitive to general structure
    - disadvantage: slow if computed without binning, large sensitivity to noise compared to MIC/TIC (example: low values obtained for the circle view of the pipe), sensitive to rotation
    - NOTE: I have not tested this much, should probably add binned version to timing plot
- MINE (MIC, TIC):
    - advantages: general, good discrimination even with noisy structures
    - disadvantages: slow to compute, noisy for small datasets, TIC sensitive to rotation
...
\hline
Index & smooth & rotation invariant & fast & general \\
\hline
Convex & ! & ! & \checkmark & ! \\
Skinny & ! & ! & \checkmark & ! \\
Stringy & $\times$ & ! & \checkmark & ! \\
\hline
splines2D & \checkmark & $\times$ & \checkmark & ! \\
dcor2D & \checkmark & $\times$ & ! & ! \\
\hline
MINE & ! & ! & ! & \checkmark\\
\hline
\end{tabular}
