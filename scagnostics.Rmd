...
Three families of data simulations are used for examining the behaviour of the index functions. Each generates  structure in two variables, with the remaining variables containing various types of noise. This is a very simple construction, because there is no need for projection pursuit to find the structure, one could simply use the scagnostics on pairs of variables. However, it serves the purpose to also evaluate the PPIs. The three data families are:

- **pipe**: $n$ points where all $p$ directions are independently drawn from a uniform distribution between $[-1,1]$, but enforcing $x_p^2 + x_{p-1}^2 = 1 \pm 0.1$ by rejection sampling as an example of a dataset with special view that is not described by functional dependence, and where sampling boundaries are apparent in the distribution, labelled "pipe"
- **sine**:  $n$ points where $p-1$ directions are independently drawn from a normal distribution with mean 0 and variance 1, and with $x_p = \sin(x_{p-1}) + \mathrm{jittering}$ as an example dataset with special view described by a function, labelled "sine"
- **spiral**:  $n$ points where $p-2$ directions are independently drawn from a normal distribution, and direction $p-1$ and $p$ are sampled on an Archimedean spiral, i.e. $r = a + b \theta$, with $a=b=0.1$ and we sample angles $\theta$ from a normal distribution with mean 0 and variance $2\pi$, giving a spiral with higher densities at lower radii. We take the absolute value of $\theta$ to fix the direction of the spiral shape when sampling the points.

Two samples sizes are also used: $n=100$ (small) and $n=1000$ (large).
...
- tour between pairs of variables, considering a planned tour that moves from the $x_1 - x_2$ view either to the $x_3 - x_4$ view (no special structure encountered over the full path) or to the $x_5 - x_6$ (moving from uninformed to special view), see Section \ref{sec:smooth}
- optimisation path, starting from $x_1 - x_2$, how can we use the available index functions and optimisation algorithms to identify the special view, see Section \ref{sec:guided}
...
```{r dataPlotsV1V2, fig.width=6, fig.height=7, fig.cap="Projection of all data sets considered onto the first two parameters $x_1$ and $x_2$, i.e. showing a typical uninformed view for each dataset."}
pL <- list()
i <- 1
...
```{r dataPlotsV5V6, fig.width=6, fig.height=7, fig.cap="Projection of all data sets considered onto the last two parameters $x_5$ and $x_6$, i.e. showing the special view built into the datasets."}
pL <- list()
i <- 1
pLabels <- c("Spiral 100", "Spiral 1000", "Pipe 100", "Pipe 1000", "Sine 100", "Sine 1000")
for(ds in list(spiral100, spiral1000, pipe100, pipe1000, sin100, sin1000)){
...
indexOverview <- tibble(convex=numeric(),
               ...
               name=character(),
               vars=character())
dsL <- list(spiral100, spiral1000, pipe100, pipe1000, sin100, sin1000)
for(ds in dsL){
...
\textcolor{red}{Maybe it will be better to combine the two traces? Can have tour from V1-V2 to V3-V4 to V5-V6, indicate position of V3-V4 in path, will save space, make direct comparison easier.}
...
```{r plotV1V2toV3V4, fig.width=6, fig.height=7, fig.cap="Short tour paths of uninformed views, moving from the projection onto $x_1$-$x_2$ to the projection onto $x_3$-$x_4$, for all considered datasets and index functions. Small samples (n=100) are shown in red, large samples (n=1000) in blue."}
...
```{r plannedtour, fig.width=6, fig.height=7, fig.cap="As Figure \\ref{fig:plotV1V2toV3V4} but moving from the projection onto $x_1$-$x_2$ to the projection onto $x_5$-$x_6$, i.e. starting from random distribution view, and moving into the special view."}
...
#spiralTour <- save_history(spiralResc, guided_tour(cmass))
