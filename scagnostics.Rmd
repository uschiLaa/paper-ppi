...
Our main motivation stems from particle physics applications where such type of association is commonly implied by experimental and theoretical constraints. A well-studied example would be the so-called CKM triangle. Here an instructive parametrisation is known, and measurements can pin-point the physical value of the parameters. Fig. \ref{fig:CKM} (left) shows a schematic view of the 2-parameter dependence for various constraints. Fig. \ref{fig:CKM} (right) shows the current state of the art constraints.
We expect this type of relations are common in particle physics, and their detection will aid understanding of the relevant physics, and as a consequence improve modelling and parametrisation. The task of uncovering the relevant features is however becoming more challenging as more complicated models are fit to larger sets of experimental results.

\begin{figure}
\includegraphics[width=0.5\textwidth]{figure/CKM-0104062.pdf}
\includegraphics[width=0.5\textwidth]{figure/rhoeta_large_ckmfitter.png}
\caption{CKM triangle constraints, schematic view shown on the left, taken from \cite{Hocker2001xe}, current picture shown on the right, from \cite{Charles2004jd}.}\label{fig:CKM}
\end{figure}
...
To get an overview of the dataset we show views of the first two parameters showing the independent distribution and the last two parameters showing the special view. The enforces relationships will also affect the overall distribution as can be seen from (fig:dataPlotsDensity).
