...
* Information based index functions from @Reshef1518 are based on the mutual information $I$ when considering the distribution induced by the data points $D \subset \mathbb{R}^2$ on an $x-$by$y$ grid $G$. With $I^{\ast} (D, x, y) = \max I (D|_G)$ the characteristic matrix $M$ is defined as \footnote{We note that all descriptions are idealized, while in practice approximations and binning are used to obtain reasonable computing time, we refer to original references and package documentation for details.}
\begin{equation}
...
No clear interpretation without trace for comparison? See e.g. TIC for path2 pipe vs sine
...
There is some issue still with search\_better not actually giving monotonically increasing results, my guess is it is because of rigid assignment of x and y axis, i.e. calculating index on starting plane, compare to randomly selected new planes, if new index > starting index and with step size = inf (default when using save history) this means we move to the new plane, but in the interpolation framework, i.e. may be rotated wrt new plane found by search\_better, and since now the distribution is the same, but x/y values are not, we get somewhat different values, this may result in somewhat smaller values than anticipated in the new starting plane, which may result in small drops rather than monotonic increase.
...
```{r bbhGuided, results="hide", fig.cap="Final view identified optimising the TIC index for the posterior obtained in the BBH merger simulation study."}
bbhM <- rescale(select(bbhD,-m2))
if(!file.exists("cache/bbhGuided.rda")){
  bbhTour <- save_history(bbhM, guided_tour(mineIndex("TIC")))
  bbhTourFull <- as.list(interpolate(bbhTour))
  save(bbhTour, bbhTourFull, file = "cache/bbhGuided.rda")
} else {
  load("cache/bbhGuided.rda")
}
iLast <- length(bbhTourFull)
fProj <- bbhTourFull[[iLast]]
dProj <- as.tibble(bbhM %*% fProj)
ggplot(dProj, aes(V1,V2)) + geom_point() + theme(aspect.ratio=1)
```

Indeed the TIC index guided tour found an informative view between the three parameters time, ra and dec whithin the full datase (after once again removing the second mass parameter, see explanation above), which is not described by a function. We can approximate it by showing the sum of the rescaled ra and dec variable against time, or when using the original parameter values by approximate scaling, see Figure \ref{fig:bbhRes}, but clearly the view in Fig. \ref{fig:bbhGuided} has higher TIC score. ((interesting to show guided tour animation here? draw axes on final view?))

```{r bbhRes, fig.cap="Approximating the final view found in the TIC guided tour optimisation by simple parameter combinations."}
ggplot(as.tibble(bbhM), aes(time, ra+dec)) + geom_point()
ggplot(bbhD, aes(time, ra+pi*dec)) + geom_point()
```
