...
-->
A projection pursuit index (ppi) is a scalar function $f$ defined on an $n$-dimensional data distribution. Typically the definition is such that larger values of $f$ indicate more interesting distribution, and therefore maximising $f$ over all possible $n$ dimensional projections of a data set with $p>n$ parameters will find the most interesting projections. The standard approach is then to map the empirical data distribution onto a density estimate for which the departure from a normal distribution can be characterised in various ways, see e.g. @CBC93. When departing from the idea of only characterising differences to the normal distribution it may however be preferred to work directly with the empirical data distribution to better capture the true distribution without bias from fitting a densitiy function.

## Standardization
scaling/sphering of data? what about outlier removal? it is build in for calculation of scagnostics measures, but not the other index functions, can generate "fake linear behaviour" by highlighting direction with outliers
consider using log transformed parameter as appropriate? e.g. timeseries examples...

## Optimization
Given a ppi we are confronted with the task of finding the maximum over all possible $n$ dimensional projections. An important consideration here is the necessity to avoid getting trapped in local maxima that are only a result of sampling fluctuatins \footnote{This is not only relevant for the choice of optimisation method but also an important consideration when designing an efficient ppi.}, and @f87 suggested a two-step procedure: the first step is using a large step size to find the approximate global maximum while stepping over pseudomaxima. A second step is then starting from the projection corresponding to the approximate maximum and employing a gradient directed optimisation for the identification of the maximum. On the other hand the global maximum may not be the only projection of interest, and one may want to observe the selected views in the context of the full distribution rather than a static view. These issues are addressed when combining projection pursuit with the grand tour [@CBCH94]. In this case the properties of a suitable optimization algorithm include monotonicity of the index value, a variable step-size to avoid overshoothing and to increase the chance of reaching the nearest maximum, and a stopping criterion allowing to move out of a local maximum and into a new search region [@tourr]. A possible alogrithm is inspired by simulated annealing and has been described in @lckl2005, this has been implemented in the \texttt{search\_better} and \texttt{search\_better\_random} search functions in the tourr package. In addition the tourr package provides the \texttt{search\_geodesic} search function, which first selects a promising direction by comparing index values between a selected number of small random steps, and then optimises the function over the line along the geodesic in that direction considering projections up to $\pi/4$ away from the current view.
...
Concretely here we consider the following types of index functions (give definitions in Appendix??):

* Scagnostics
    + Convex
    + Skinny
    + Stringy
* New index function from @Grimm2016
    + Distance correlation
    + Spline based measure
* Information based index functions from @Reshef1518
