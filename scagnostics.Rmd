...
## Example 1
The model describes a neutron star merger and contains 6 free parameters, i.e. each neutron star is described by its mass $m$ and radius $R$, and a so-called tidal deform ability parameter $\Lambda$ which is a function of the mass and radius, approximately proportional to $(m/R)^{-5}$. A posterior sample containing 2538 points is available @ligoData and can be used to constrain the parameters. Here we use the "Parametrized-EoS\_nomaxmass\_posterior\_samples.dat" dataset and aim to recover the dependence between the three parameters of the better constrianed system out of a larger parameter set. For details about the modelling and statistical procedure see @Abbott2018exr. A common representation of posterior samples used for interpretation are so-called "corner plots", i.e. density displays for all 2-d parameter combinations as well as the 1-d profile, highlighting empirically evaluated regions of highest posterior density, which correspond to credibility regions at a given level. The information can similarly be presented in a scatter plot matrix using transparency to convey the density information. Since in the following we work with 2-d scatter plots we choose to show the data in this fashion, see Fig. \ref{fig:neutronStarSPLOM}.
...
To further investigate the dataset we remove the $L1$ parameter and again use the spline index to find functional dependence between the remaining four parameters. For this example, because of the strong correlation between the various parameters, we recover the dependence of $L2$ on the mass and radius of the lighter neutron star, despite having removed its mass from the parameter set. In this setting $L2$ is best described as a function of all 3 parameters still considered, see Figure \ref{fig:nseRemL1}.
...
## Example 2
The above example is using only a reduced set of the free parameters relevant to the physical interpretation, and we already know the expected result by construction. In general many additional nuissance parameters will also enter the fit, see e.g. @Smith:2016qas. We use a simulation study \textcolor{red}{simulating a binary black hole (BBH) merger event...need more info maybe, no reference but Rory offered to write short description} including the full set of nuissance parameters as a test case of larger parameter space in which we aim to detect structure. The dataset is summarised in \ref{fig:bbhSimulation} where we have removed a subset of less informative parameters to obtain a clear picture. Interesting correlations can be seen between the parameters "ra", "dec" and "time", where the first two describe the position of the BBH event in the sky, and the "time" being the merging time (in GPS units). Note in particular the relation between "dec" and "time" is not functional, which motivates us to use the TIC index for this example (drawback is however the run time, this data sample contains 10k points, making optimisation of the MINE index functions very inefficient). Some other interesting patterns can also be identified. (These patterns are representative of the likelihood function, since most priors are flat, or build such to capture growth with volume rahter than distance. This is still efficient thanks to special sampling techniques that allow jumps in parameter space.)

As a first test we use the full dataset (with exception of $m2$ for the same reasons as stated in the first example), and use different index functions. Here we expect to recover some relation between the correlated variables dec, time and ra. We compare the final view using three different metrics in Fig. \ref{fig:bbhGuided}, showing the views with (local?) maximum of the TIC index (left) spines2d index (middle) and for comparison maximum of 1-convex (right). Indeed the TIC and spline index identify dec, time and ra parameters and show views of the 2-d band that is embedded in three dimensions \textcolor{red}{maybe having tour of those three dimensions would be helpful here}. Notably, the exact views differ based on the index function selected, optimising TIC is showing us the band "from the side", resulting in clearly non-functional dependence. On the other hand, the index based on the scagnostics member convex is too noisy to find any views of interest, and the final  view is essentially equal to the starting plane in this case. \textcolor{red}{different scagnostics member may be better suited, but I expect the result will be the same. might still be useful to try skinny instead of 1-convex?}.
...
We next investigate the dependence of the final result on the input starting plane. In particular if there are many local maxima the effect may be large, and many random starting planes may be needed to find a global maximum. To this end we first reduce the dataset dimensionality to include only two types of parameters: first, spin related parameters (i.e. alpha, thetajn, chitot and chip), and second position related parameters (i.e. ra, dec and distance). Note that ra and dec are included here, for which we have established strong correlation above. We then generate a small set of random starting planes and optimise the splines index (note that it is prefered here over the TIC index as it is much more efficient to compute on large data sets). We compare six final views identified in this manner (i.e. differing only in randomly selected starting plane) in Fig. \ref{fig:bbhGuided3}. We note large variation in final views, only one of the six tries identified the view of ra vs dec. \textcolor{red}{list corresponding index values for each plot, maybe in the caption.}
```{r bbhGuided3, results="hide", fig.cap="Final views identified in the dataset considering the seven dimensional parameter space (alpha, thetajn, chitot, chip, ra, dec, distance), differing only by randomly selected starting plane."}
