...
The spiral is the most challenging structure to detect amongst the distribution considered here (compare also Posses results?). Here we explore the role of dimensionality in the index optimization, using the skinny index that was found to be sensitive to this structure.
...
In d=4 dimensions the optimization identifies a view showing the spiral after 33 steps, and we limit the optimizer to consider only 33 steps for the study in 6 and 8 dimensions for direct comparison. Note however that the spiral is typically also not identified with much larger number of steps, the limiting factor being rather the number of random projections sampled and rotation non-invariance of the index prohibiting the convergence, in particular in higher dimensions.
Results are shown in the top panel Fig.~\ref{fig:findspiral}, showing the pairwise distance of the 33 planes found in 4, 6 and 8 dimensions (left), and comparing the evolution of the distance from the plane containing the spiral view with progression of the optimization (right). We see that the pairwise distances grow with d (what do we learn from that?). Moreover we find that the distance to the special view is far from monotonically decreasing towards zero, and large jumps are possible. Note that the spiral itself is approximately rotation invariant, but partial views are not. At medium distances from the special view we often observe a jump to higher distances as consequence of in-plane rotation dependence for such partial views. This stabilizes closer to the special view, however it relies on randomly identifying a projection within this angle. As a consequence increasing dimensionality dramatically reduces the chance of identifying structures with high index values.
...
Moreover we can either select the mean value, or to avoid large pulls from sharp peaks, we can remove the most extreme values and then calculate the mean from the remaining evaluations. Only the mean will be considered here.
