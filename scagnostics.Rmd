...
2. Chamge to a tour path between a pair of nuisance variables $x_1 - x_2$ and the structured pair of variables $x_5 - x_6$, and compute the PPI along this. This examines the squintability, and smoothness. If the function is smooth and slowly increases towards the structured projection, then the structure is visible from a distance. 
3. Use the guided tour to examine the ease of optimisation. This depends on having a relatovely smooth function, with structure visible from a distance. One index is optimised to show how effectively the maximum is attained, and the values for other PPIs is examined along the same path, to examine the similarity between PPIs. 
...
The guided tour combines optimisation with interpolation between pairs of planes. Target planes of the path are chosen to maximise the PPI. There are three derivative-free optimisation methods available in the guided tour: \verb# search_better_random# (1), \verb# search_better# (2), and \verb# search_geodesic# (3). Method 1 casts a wide net randomly generating projection planes, computing the PPIs and keeping the best projection. Method 2 conducts a localised maximum search. Method 3 is quite different: a local search is conducted to determine a promising direction, and then this direction is followed until the maximum in that direction is found. For all methods the optimisation is iterative, and we record each step where a plane with increased index value has been found. The stopping condition is given as number of tries that did not yield an increased index value at the final step in the optimisation.
For method 1 and 2 two parameters control the optimisation: the search window $\alpha$, giving the maximum distance from the current plane in which projections are samples, and the cooling factor, giving the incremental decrease in search window size. Method 3 in principle also has two free parameters, which are however fixed in the current implementation. The first is the small step size used when evaluating the most promising direction, it is fixed to 0.01, and the second parameter being the window over which the line search is performed, fixet to $\pm \pi/4$ away from the current plane.
For distributions and indexes with smooth behaviour and good squinatbaility, method 3 is the most efficient method for optimisation. If these two criteria are not met the method may still be useful, but only given an informed starting point. In such cases we can follow a method similar to that proposed by Friedman, i.e. we break the optimisation in two distinct steps. A first step ("scouting") uses method 2 with large search window and no cooling as a way of stepping over fluctuations and local maxima and yielding an approximation of the global maximum (note here that this also requires large number of tries for the stopping condition in particular in high dimensions, since most randomly picked planes will not be interesting). The second step is then to use method 3 starting from the approximate maximum, which will return the global maximum.


### Looking down the pipe

As seen in Figure \ref{fig:plannedtour} the pipe structure has a small squint angle, and indeed using only method 3 does not uncover it in the six dimensional dataset. We therefore adapt the two step procedure described above. For the scouting we use $\alpha = 0.5$ and stopping condition of maximum 5000 tries, and we optimise the TIC index.

\textcolor{red}{Probably a good idea to give stopping condition to scouting phase, since at each step we may spend a long time looking for better projections. E.g. if maximum index value is 1, stop scoutig phase when reaching index value 0.9, i.e. should be in squintable distance of the index maximum, switch to geodesic optimiser at this point (only works if indeed the optimal view takes index values close to the maximum though)}

The results are shown below, first Fig. \ref{fig:pipeFirstRun} shows how the considered index values evolve in the scouting phase.
\textcolor{red}{Updated the plot, makes it look like skinny, 1-convex should be better index to optimise, however in practice this is not the case, probably because TIC is less sensitive to fluctuations especially also under rotation, making it more robust against getting stuck in local maxima. Maybe it will be better to leave scales free for this plot, since increase in TIC is very hard to see currently. Or just remove plot? not actually that intersting...}
...
Given the results from Fig.\ref{fig:plannedtour} we expect to be able to efficiently identify the sine view in the 6-d distribution, using only method 3 for the optimisation, using e.g. the splines2d index. Indeed the guided tour quickly converges in this case, the evolution of the index functions is shown in Fig \ref{fig:findsine}. The only difficulties stem from the rotation dependence of the index function as well as the structured distribution, this will be discussed in some more details below. Here we only note that even for anchor planes highlighted by blue vertical lines the index sometimes decreases because of this dependence. These are however not dramatic effects because this search method is closely related to how interpolation is done (a different picture would be optained using method 2 which often leads to large drops in index values after the interpolation step).
...
```{r findsine, fig.width=6, fig.height=7, fig.cap="Guided tour optimising the splines2d index for the Sine 1000 dataset. Anchor planes are marked by the blue vertical lines.", results="hide"}
set.seed(2018)
...
The spiral is the most challenging structure to detect amonst the distribution considered here (compare also Posses results?). Here we explore the role of dimensionality in the index optimisation, using the skinny index that was found to be sensitive to this structure.
We use a setup similar to that introduced for detecting the pipe structure, i.e.\ we use the search\_better optimisation of the guided tour, disabeling the cooling and with a large search window, while sampling a large number of random projections (up to 5000) for each step.
We consider distributions in d=4,6,8 dimensions and compare features found in the tour.
In d=4 dimensions the optimisation identifies a view showing the spiral after 33 steps, and we limit the optimiser to consider only 33 steps for the study in 6 and 8 dimensions for direct comparison. Note however that the spiral is typically also not identified with much larger number of steps, the limiting factor being rather the numver of random projections sampled and rotation non-invariance of the index prohibiting the convergence, in particular in higher dimensions.
Results are shown in the top panel Fig.~\ref{fig:findspiral}, showing the pariwise distance of the 33 planes found in 4, 6 and 8 dimensions (left), and comparing the evolution of the distance from the plane containing the spiral view with progression of the optimisation (right). We see that the pairwise distances grow with d (what do we learn from that?). Moreover we find that the distance to the special view is far from monotonically decreasing towards zero, and large jumps are possible. Note that the spiral itself is approximately rotation invariant, but partial views are not. At medium distances from the special view we often observe a jump to higher distances as consequence of in-plane rotation dependence for such partial views. This stabilises closer to the special view, however it relies on randomly identifying a projection within this angle. As a consequence increasing dimensionality dramatically reduces the chance of identifying structures with high index values.


```{r findspiral, out.width="\\textwidth", fig.cap="Guided tour optimising the skinny index for the Sprial dataset with 1000 datapoints, with d = 4, 6, 8. The top left plot shows the distribution of pairwise distances between planes obtained via the guided tour, the top right shows the evolution of distance to the plane containing the spiral as the index is being optimised. The bottom row shows the final view obtained for d = 4 (left), d = 6 (middle) and d = 8 (right) after 33 steps in the guided tour.", results="hide"}
...
if(!file.exists("cache/findspiral.rda")){
  ...
  for(d in c(4,6,8)){
    ...
    spiralD <- spiralData(d, 1000) %>% scale() %>% as_tibble()
    spiralTour <- save_history(spiralD, guided_tour(scagIndex("Skinny"), search_f = tourr:::search_better,
                                                    max.tries = 5000, cooling = 1, alpha = pi),
                              max_bases = maxBases)
    if(d==4){maxBases <- min(maxBases, length(spiralTour))}
    distDf <- rbind(distDf, tibble(d=d, distance=distanceDist(spiralTour)))
    distToSp <- rbind(distToSp, tibble(d=d, distance=distanceToSp(spiralTour, specialPlane), t=1:maxBases))
    fView <- as.tibble(as.matrix(spiralD) %*% as.list(spiralTour)[[maxBases]])
...
p1 <- ggplot(distDf, aes(group=d, y=distance, x=d)) + geom_boxplot()
p2 <- ggplot(distToSp, aes(color=as.factor(d),x =t, y=distance)) +
  geom_point() +
  labs(y = "Distance to special view", color = "d")
...
We study rotation dependence using the sine function distribution, and tracing the index values as a function of roation angle. Results are shown in Figure \ref{fig:rotationDep}. Several indexes are found to be invariant, holes, convex and MIC. The dcor2d, splines2d and TIC index show consistent patterns in rotation dependence, on the other hand the scagnostics index skinny is found to have sizable random fluctuations as the basis is rotated.
