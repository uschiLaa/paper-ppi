...
bbhM3 <- rescale(select(bbhD, alpha, chi_tot, chi_p, ra, dec, distance))
if(!file.exists("cache/bbhGuidedSpline3.rda")){
  set.seed(2018)
  bbhTour23 <- save_history(bbhM3, guided_tour(splineIndex()))
  bbhTourFull23 <- as.list(interpolate(bbhTour23))
  save(bbhTour23, bbhTourFull23, file = "cache/bbhGuidedSpline3.rda")
} else {
  load("cache/bbhGuidedSpline3.rda")
}
iLast <- length(bbhTourFull23)
fProj2 <- bbhTourFull23[[iLast]]
dProj <- as.tibble(bbhM3 %*% fProj2)
p1 <- ggplot(dProj, aes(V1,V2)) + geom_point(alpha = 0.02) + theme(aspect.ratio=1)
p2 <- ggplot(as.tibble(bbhM3), aes(ra, dec)) + geom_point(alpha = 0.02) + theme(aspect.ratio=1)
grid.arrange(p1, p2, ncol=2)
```
