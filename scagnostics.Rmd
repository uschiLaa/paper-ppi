...
\textcolor{red}{Holes and cmass index require standardisation of data, in general PCA, here rescaling to mean zero, standard deviation one should be enough, need to investigate implementation for standardisation in the background. Should standardisation be used when working with new indices as well?}
...
```{r neutronStarEq,  results="hide", fig.height=3, fig.cap="Final view when optimising the splines2d index for the neutron star merger dataset, showing the functional dependence between the mass, radius and lambda parameter of one of the neutron stars."}
set.seed(2018)
nsD <- read_csv("data/samples.csv") 
nsM <- nsD %>%
