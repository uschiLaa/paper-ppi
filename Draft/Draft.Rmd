...
title:
  formatted: "Using Scagnostics to find Interesting Projections of Multivariate Data"
...
keywords:
  ...
  formatted: [scagnostics, tour, projection pursuit, multivariate data, statistical graphics, data visualisation]
...
```{r motivation}
library(mvtnorm)
x1 <- rmvnorm(100, mean=c(0,0), 
             sigma=matrix(c(1, 0.8, 0.8, 1), ncol=2, byrow=T))
x2 <- rmvnorm(100, mean=c(3,0), 
             sigma=matrix(c(1, 0.8, 0.8, 1), ncol=2, byrow=T))
x3 <- rmvnorm(100, mean=c(0,3), 
             sigma=matrix(c(1, 0.8, 0.8, 1), ncol=2, byrow=T))
df <- data.frame(x1=rnorm(300),
                 x2=rnorm(300),
                 x3=rnorm(300),
                 x4=rnorm(300), 
                 x5=scale(c(x1[,1], x2[,1], x3[,1])), 
                 x6=scale(c(x1[,2], x2[,2], x3[,2])), 
                 cl=c(rep("A",100), rep("B", 100), rep("C",100)))
ggplot(df, aes(x=x5, y=x6, colour=cl)) + geom_point() + theme(aspect.ratio=1)
```

```{r eval=FALSE}
library(tourr)
quartz()
col <- c(rep("#f0a800", 100), rep("#486030", 100), rep("#c03018", 100))
animate(df[,1:6], guided_tour(lda_pp(df$cl)),
        display_xy(axes="bottomleft", col=col))
```

```{r eval=FALSE}
numbat <- read_csv("data/numbat.csv") %>% filter(group=="A")
library(geozoo)
x <- sphere.hollow(p=2, n=2000)$points
colnames(x) <- c("x4", "x7")
x <- data.frame(x1=scale(rnorm(2000)), 
                x2=scale(rnorm(2000)), 
                x3=scale(rnorm(2000)),
                x4=scale(x[,1]), 
                x5=scale(rnorm(2000)), 
                ...
                x8=scale(rnorm(2000)), 
                x9=scale(rnorm(2000)),
                x10=scale(rnorm(2000)), group="B")
numbat <- bind_rows(numbat, x)
ggplot(numbat, aes(x=x4, y=x7, col=group)) + 
  geom_point() + facet_wrap(~group) + scale_colour_manual(values=c("#609048", "#c03018"))
write_csv(numbat, path="data/numbat_circle.csv")
```

```{r eval=FALSE}
library(tourr)
quartz()
col <- c(rep("#f0a800", 100), rep("#609048", 2100), rep("#c03018", 2000))
animate(numbat[,1:10], 
        guided_tour(lda_pp(numbat$group), search_f = search_better_random),
        display_xy(axes="bottomleft", col=col))
```
...
In addition the computing time is also important, as it might limit the dynamic viewing and require to first record a guided tour path which can later be viewed in a smooth display.
...
Maybe include GGally pairs plot here?
...
```{r scagnostics-summary, fig.height= 14, fig.width=9, fig.cap = "(ref:scagnostics-summary)", eval=FALSE}
```
...
```{r scatter-binning, eval=FALSE, fig.height=2, fig.cap = "(ref:scatter-binning)"}
```

```{r scatter-table, eval=FALSE}
```
...
```{r scagnositcs-timing, eval=FALSE, fig.cap = "(ref:scagnostics-timing)"}
```
...
```{r smoothing, eval=FALSE, fig.cap = "(ref:smooting)"}
```
...
```{r guided-tour, eval=FALSE, include=FALSE}
```
```{r guided-tour-plot, eval=FALSE, fig.height = 3, fig.cap = "(ref:guided-tour-plot)"}
```
...
```{r guided-tour-scatter, eval=FALSE, fig.height = 3, fig.cap = "(ref:guided-tour-scatter)"}
```
...
Additions to the standard scagnostics measures have been implemented in the package mbgraphics, based on splines (spline2d) and distance correlation (dcor2d).
Give definitions here..
We repeat the study from before, we first test in Figure ADDREF the behaviour of the index values over 100 interpolated projections that are randomly selected via the grand tour algorithm (the same projections as considered above).
Note that here the measures are calculated on non-binned data.
The general behaviour seems similar to the scagnostics monotonic index.
The observed behaviour is smooth (much in contrast to the behaviour of the scagnostics measures) and we next test the behaviour in a guided tour, comparing again also the first and final projection. (which index to select, will try both below...)
It seems they both maximise correlation, this might not be too useful?
Seems like [,1] basically -[,2] in the final projection (except for [7,])?
probably we see only structure of gminus2 variable, which has very narrow peak, only few outlying points
is there a better way of transforming it to spread out the structure?

```{r mbgraphics-indices, eval=FALSE, fig.height=3, fig.cap="(ref:mbgraphics-indices)"}
```
```{r guided-spline2d, eval=FALSE, include=FALSE}
```
```{r guided-spline2d-plot, eval=FALSE, fig.height=3, fig.cap="(ref:guided-spline2d-plot)"}
```
```{r guided-dcor2d, eval=FALSE, include=FALSE}
```
```{r guided-dcor2d-plot, eval=FALSE, fig.height=3, fig.cap="(ref:guided-dcor2d-plot)"}
```
...
```{r gminus2-box, eval=FALSE, fig.height=3}
```

```{r guided-dcor2d-no-gminus2, eval=FALSE, include=FALSE}
```
```{r guided-dcor2d-plot-no-gminus2, eval=FALSE, fig.height=3, fig.cap="(ref:guided-dcor2d-plot)"}
```
