...
**As we now outline in the section overview, this paper first investigates the behaviour of the new indexes to understand the potential and challenges in using them in projection pursuit settings, before applying them to examples of the physics problems we are interested in addressing. While we agree that the simulation study is central to the work, we believe that showing applications of the method to unknown distributions is also relevant for the COST audience. Note that the application section now also contains guidelines on how to apply projection pursuit to new datasets, which are referenced in comments in the source code producing the examples, making them more useful to the reader** XXX check
...
**We have added the missing references, additional definitions are not required.** XXX check
