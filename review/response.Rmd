...
**As we now mention in the discussion, the easiest way for new users is to access these through the graphical interface available in the galahr package. The package documentation includes a step-by-step example using the guided tour.** XXX check! UP TO HERE!
...
**We are considering seven new index functions. In Section 3 we also include the holes index for comparison, as is now explained in the end of Section 2.2. The holes index is missing in Table 3 which summarises our findings on the new index functions, in addition we have combined the MINE indexes (MIC, TIC) that have very similar behaviour in all assessments, resulting in a list of six indexes. This is now also explained in the text. Finally Section 4.2.2 only considers the new index functions.**

7) I would suggest moving the appendix A (the description of "holes/cmass" index) to Section 2.2 for readability.

**As mentioned above we now mention the holes index and point to the relevant references in Section 2.2. However, we believe the explanation of the rescaling should remain in the appendix.**
...
**(In my opinion all of this was already very clear before. When discussing this before you suggested making the Figure caption more clear, but it seems they are already very clear and I would not want to make them longer.)** XXX check and remove
...
**(I would list: demonstrate potential as PPI, understand shortcomings, and based on that develop strategies, e.g. the two-step optimisation. Add something like this in Section 3.4?)** XXX add?
...
**(Point to initial motiviation, maybe say that physics application presents more realistic example with interesting structures in non-trivial projections)** XXX
